...
public:
    GUI_Manager(EventManager* l_eventMgr, SharedContext* l_context);
    ~GUI_Manager();
    ...
    template <class T>
    void RegisterElement(const GUI_ElementType& l_type) {
        m_guiFactory[l_type] = [](GUI_Interface* l_owner)->GUI_Element*{
            return new T("", l_owner);
        };
    };
private:
    GUI_Element* CreateElement(const GUI_ElementType& l_type, GUI_Interface* l_owner);
    bool LoadStyle(const std::string& l_file, GUI_Element* l_element);
    GUI_ElementType StringToType(const std::string& l_type);
