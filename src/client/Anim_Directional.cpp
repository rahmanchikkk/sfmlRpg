...
void Anim_Directional::CropSprite() {
    ...
    sf::IntRect rect((m_spriteSheet->GetSpriteSize().x * m_frameCurrent) + padding.x + (spacing.x * m_frameCurrent),
        ((m_spriteSheet->GetSpriteSize().y * ((short)m_spriteSheet->GetDirection() + m_frameRow)) +
...
void Anim_Directional::FrameStep() {
    bool b = SetFrame(m_frameStart < m_frameEnd ? (m_frameCurrent + 1) : (m_frameCurrent - 1));
    if (b) return;
    if (m_loop) {
...
void Anim_Directional::ReadIn(std::stringstream& l_stream) {
    l_stream >> m_frameStart >> m_frameEnd >> m_frameRow >> m_frameTime >>
